...
pub unsafe extern "C" fn ptrhash_new_u64(
    ...
    // Determine hash function to use
    let hash_function = if params.has_override_hash_function != 0 {
        params.override_hash_function
    } else {
        0  // Default to FxHash if not overridden
    };
    
    
    
    match std::panic::catch_unwind(|| {
        build_u64_ptrhash(keys_slice, hash_params, hash_function)
    }) {
        Ok(Ok(ptr)) => {
            let drop_fn = match get_u64_drop_fn(hash_function) {
                Ok(f) => f,
